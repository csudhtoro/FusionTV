...
public class WarPoliticsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<TVShowModel> mData;
    private OnShowListener onGenreListener;

    public WarPoliticsAdapter(OnShowListener onShowListener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new WarPoliticsViewHolder(view, onGenreListener);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ((WarPoliticsViewHolder)holder).title.setText(mData.get(position).getName());
        Glide.with(((WarPoliticsViewHolder) holder).itemView.getContext())
                .load("https://image.tmdb.org/t/p/w500/"+mData.get(position)
                        .getPoster_path())
                .into(((WarPoliticsViewHolder)holder).imageView);
    }
