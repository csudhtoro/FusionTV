...
public class FamilyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FamilyAdapter(OnShowListener onShowListener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new FamilyViewHolder(view, onGenreListener);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ((FamilyViewHolder)holder).title.setText(mData.get(position).getName());
        Glide.with(((FamilyViewHolder) holder).itemView.getContext())
                .load("https://image.tmdb.org/t/p/w500/"+mData.get(position)
                        .getPoster_path())
                .into(((FamilyViewHolder)holder).imageView);
    }
