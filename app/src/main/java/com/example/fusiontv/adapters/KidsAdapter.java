...
public class KidsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public KidsAdapter(OnShowListener onShowListener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new KidsViewHolder(view, onGenreListener);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ((KidsViewHolder)holder).title.setText(mData.get(position).getName());
        Glide.with(((KidsViewHolder) holder).itemView.getContext())
                .load("https://image.tmdb.org/t/p/w500/"+mData.get(position)
                        .getPoster_path())
                .into(((KidsViewHolder)holder).imageView);
    }
