...
public class SciFiFantasyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<TVShowModel> mData;
    private OnShowListener onGenreListener;

    public SciFiFantasyAdapter(OnShowListener onShowListener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new SciFiFantasyViewHolder(view, onGenreListener);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ((SciFiFantasyViewHolder)holder).title.setText(mData.get(position).getName());
        Glide.with(((SciFiFantasyViewHolder) holder).itemView.getContext())
                .load("https://image.tmdb.org/t/p/w500/"+mData.get(position)
                        .getPoster_path())
                .into(((SciFiFantasyViewHolder)holder).imageView);
    }
