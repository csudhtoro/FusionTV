...
public class TalkAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public TalkAdapter(OnShowListener onShowListener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new TalkViewHolder(view, onGenreListener);
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ((TalkViewHolder)holder).title.setText(mData.get(position).getName());
        Glide.with(((TalkViewHolder) holder).itemView.getContext())
                .load("https://image.tmdb.org/t/p/w500/"+mData.get(position)
                        .getPoster_path())
                .into(((TalkViewHolder)holder).imageView);
    }
