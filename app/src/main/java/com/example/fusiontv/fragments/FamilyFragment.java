...
import com.example.fusiontv.adapters.FamilyAdapter;
...
public class FamilyFragment extends Fragment implements OnShowListener {

    private ShowListViewModel showListViewModel;

    RecyclerView familyRecyclerview;
    private FamilyAdapter familyRecyclerViewAdapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        familyRecyclerview = (RecyclerView) getView().findViewById(R.id.family_recyclerview);


        showListViewModel.searchShowByFamily(10751, 1);
        PutFamilyDataIntoRecyclerView();
        ObserveGenreChange();
    }

    private void PutFamilyDataIntoRecyclerView() {
        familyRecyclerViewAdapter = new FamilyAdapter(this);

        SpacingRV rvDecorator = new SpacingRV(-35, -35);
        familyRecyclerview.addItemDecoration(rvDecorator);

        familyRecyclerview.setAdapter(familyRecyclerViewAdapter);
        familyRecyclerview.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, false));

        //RecyclerView Pagination
        //Loading next page of api response
        familyRecyclerview.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void ObserveGenreChange() {
        showListViewModel.getShowsByFamily().observe(getViewLifecycleOwner(), new Observer<List<TVShowModel>>() {
            @Override
            public void onChanged(List<TVShowModel> tvShowModels) {
                familyRecyclerViewAdapter.setmShows(tvShowModels);
            }
        });
    }
