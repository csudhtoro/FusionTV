...
import com.example.fusiontv.adapters.DramaAdapter;
...
public class DramaFragment extends Fragment implements OnShowListener {

    private ShowListViewModel showListViewModel;

    RecyclerView dramaRecyclerview;
    private DramaAdapter dramaRecyclerViewAdapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        dramaRecyclerview = (RecyclerView) getView().findViewById(R.id.drama_recyclerview);


        showListViewModel.searchShowByDrama(18, 1);
        PutDramaDataIntoRecyclerView();
        ObserveGenreChange();
    }

    private void PutDramaDataIntoRecyclerView() {
        dramaRecyclerViewAdapter = new DramaAdapter(this);

        SpacingRV rvDecorator = new SpacingRV(-35, -35);
        dramaRecyclerview.addItemDecoration(rvDecorator);

        dramaRecyclerview.setAdapter(dramaRecyclerViewAdapter);
        dramaRecyclerview.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, false));

        //RecyclerView Pagination
        //Loading next page of api response
        dramaRecyclerview.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void ObserveGenreChange() {
        showListViewModel.getShowsByDrama().observe(getViewLifecycleOwner(), new Observer<List<TVShowModel>>() {
            @Override
            public void onChanged(List<TVShowModel> tvShowModels) {
                dramaRecyclerViewAdapter.setmShows(tvShowModels);
            }
        });
    }
