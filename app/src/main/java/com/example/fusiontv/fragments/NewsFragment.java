...
import com.example.fusiontv.adapters.NewsAdapter;
...
public class NewsFragment extends Fragment implements OnShowListener {

    private ShowListViewModel showListViewModel;

    RecyclerView newsRecyclerView;
    private NewsAdapter newsRecyclerViewAdapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        showListViewModel.searchShowByNews(10763, 1);
        PutNewsDataIntoRecyclerView();
        ObserveGenreChange();
    }

    private void PutNewsDataIntoRecyclerView() {
        newsRecyclerViewAdapter = new NewsAdapter(this);

        SpacingRV rvDecorator = new SpacingRV(-35, -35);
        newsRecyclerView.addItemDecoration(rvDecorator);
    ...
    private void ObserveGenreChange() {
        showListViewModel.getShowsByNews().observe(getViewLifecycleOwner(), new Observer<List<TVShowModel>>() {
            @Override
            public void onChanged(List<TVShowModel> tvShowModels) {
                newsRecyclerViewAdapter.setmShows(tvShowModels);
            }
        });
    }
