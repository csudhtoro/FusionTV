...
import com.example.fusiontv.adapters.MysteryAdapter;
...
public class MysteryFragment extends Fragment implements OnShowListener {

    private ShowListViewModel showListViewModel;

    RecyclerView mysteryRecyclerView;
    private MysteryAdapter mysteryRecyclerViewAdapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mysteryRecyclerView = (RecyclerView) getView().findViewById(R.id.mystery_recyclerview);


        showListViewModel.searchShowByMystery(9648, 1);
        PutMysteryDataIntoRecyclerView();
        ObserveGenreChange();
    }

    private void PutMysteryDataIntoRecyclerView() {
        mysteryRecyclerViewAdapter = new MysteryAdapter(this);

        SpacingRV rvDecorator = new SpacingRV(-35, -35);
        mysteryRecyclerView.addItemDecoration(rvDecorator);

        mysteryRecyclerView.setAdapter(mysteryRecyclerViewAdapter);
        mysteryRecyclerView.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, false));

        //RecyclerView Pagination
        //Loading next page of api response
        mysteryRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void ObserveGenreChange() {
        showListViewModel.getShowsByMystery().observe(getViewLifecycleOwner(), new Observer<List<TVShowModel>>() {
            @Override
            public void onChanged(List<TVShowModel> tvShowModels) {
                mysteryRecyclerViewAdapter.setmShows(tvShowModels);
            }
        });
    }
