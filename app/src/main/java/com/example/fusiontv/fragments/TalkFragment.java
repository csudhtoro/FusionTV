...
import com.example.fusiontv.adapters.TalkAdapter;
...
public class TalkFragment extends Fragment implements OnShowListener {

    private ShowListViewModel showListViewModel;

    RecyclerView talkRecyclerView;
    private TalkAdapter talkRecyclerViewAdapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        talkRecyclerView = (RecyclerView) getView().findViewById(R.id.talk_recyclerview);


        showListViewModel.searchShowByTalk(10767, 1);
        PutTalkDataIntoRecyclerView();
        ObserveGenreChange();
    }

    private void PutTalkDataIntoRecyclerView() {
        talkRecyclerViewAdapter = new TalkAdapter(this);

        SpacingRV rvDecorator = new SpacingRV(-35, -35);
        talkRecyclerView.addItemDecoration(rvDecorator);

        talkRecyclerView.setAdapter(talkRecyclerViewAdapter);
        talkRecyclerView.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL, false));

        //RecyclerView Pagination
        //Loading next page of api response
        talkRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void ObserveGenreChange() {
        showListViewModel.getShowsByTalk().observe(getViewLifecycleOwner(), new Observer<List<TVShowModel>>() {
            @Override
            public void onChanged(List<TVShowModel> tvShowModels) {
                talkRecyclerViewAdapter.setmShows(tvShowModels);
            }
        });
    }
